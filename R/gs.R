gs <-
function(rhoInput,MInput,dateInput){
...
norm.cop=list()
tao=matrix(NA,d,M)
# Settlement Date 设定计算日
CompDate=list()
CompDate=as.Date(dateInput) # 设定计算日Settlement Dateeeeeeeeeeeeeeeeeeeeeeeeeeeee
#lambda=lambdaInput
lambda=numeric()

lambdaVector=read.csv("C:/+++++CDODATA/usingDataSet/defIntensity.csv")
...
#---1) 2)Compute tao 计算违约时间
norm.cop <- normalCopula(rho, dim = d, dispstr = "ex") # 用copula抽取随机数
#U <- rCopula(M, norm.cop) # 完成default time矩阵赋值,抽出125个随机数Ui,i=1,...,125
...
#Time=as.numeric(as.Date(pa)-as.Date(tStStart # 至未来各付款期payDay与始发期tStart的时间距离
TT=length(timeTable[,1]) # TT为未来付款期的次数
tPercent=numeric()
